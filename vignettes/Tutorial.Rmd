...
`toy_lm()` has larger itr/sec than `lm()` because it is designed as a simplified linear regression function. It only accepts numeric predictors and uses the normal equations to compute coefficients, avoiding the more general and computationally heavier techniques used by lm().
```{r}
bench::mark(
